...
class KeycloakOpenidConnectTestCase(TestCase):
    def setUp(self):
        ...
        self.openid_client = KeycloakOpenidConnect(
            realm=self.realm, client_id=self.client_id, client_secret=self.client_secret
        )
    ...
    def test_decode_token(self, patched_jwt):
        ...
        patched_jwt.decode.assert_called_once_with(
            "test-token", "test-key", algorithms=["RS256"], audience=self.client_id
        )
    ...
    def test_userinfo(self):
        ...
        self.realm.client.get.assert_called_once_with(
            "https://userinfo", headers={"Authorization": "Bearer token"}
        )
        self.assertEqual(result, self.realm.client.get.return_value)
    ...
    def test_client_credentials(self):
        response = self.openid_client.client_credentials(scope="scope another-scope")
        self.realm.client.post.assert_called_once_with(
            "https://token",
            data={
